...
const connectCeloWallet = async function () {
	if (window.celo) {
		notification(
			"Dapp is requesting permission to connect to your wallet: "
		);
		try {
...
const getItems = async function () {
	const _numberOfItemsAvailable = await contract.methods
		.totalAccessoriesAvailable()
		.call();
	const _items = [];
...
document.querySelector("#newItemBtn").addEventListener("click", async (e) => {
	const params = [
	...
	notificationSuccess(
		`congratulations: "${params[0]}" added successfully 🎉`
	);
	getItems();
});
...
document.querySelector("#marketplace").addEventListener("click", async (e) => {
	if (e.target.className.includes("buyBtn")) {
		...
		const units = Number(prompt("How many pieces do you want?"));
		if (typeof units !== "number") {
			notification("Units entered is not a valid number");
			return;
		}
		if (units > items[index].units) {
			notification(
				"Not enough units availalbe in stock to fulfill this order"
			);
			return;
    }
			try {
				notification("⌛ Waiting for payment approval...");
				await approve(items[index].price.multipliedBy(units));
			} catch (error) {
				notificationError(`⚠️ ${error}.`);
			}
			notification(
				`⌛ Awaiting payment for ${units} piece(s) of "${items[index].name}"...`
			);
			try {
				const result = await contract.methods
					.buyItem(index, units)
					.send({ from: kit.defaultAccount });
				notificationSuccess(
					`congratulations : You successfully bought ${units} piece(s) of "${items[index].name}". `
				);
				getItems();
				getBalance();
			} catch (error) {
